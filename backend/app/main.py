...
async def lifespan(app: FastAPI):
    ...
    try:
        from app.api.comics import mark_orphaned_batch_tasks_after_startup

        orphaned_summary = mark_orphaned_batch_tasks_after_startup()
        marked_total = orphaned_summary["comic_tasks_marked"] + orphaned_summary["pipeline_tasks_marked"]
        if marked_total > 0:
            logger.warning("已清理容器重启遗留的批量任务: %s", orphaned_summary)
    except Exception as exc:
        logger.error("清理容器重启遗留批量任务失败: %s", exc, exc_info=True)
...
app.include_router(character_images.router, prefix="/api")
app.include_router(careers.router, prefix="/api")  # 职业管理API
app.include_router(chapters.router, prefix="/api")
app.include_router(comics.router, prefix="/api")  # 漫画管理API
