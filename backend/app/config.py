...
# 数据库配置从环境变量读取。公开仓库不提供真实连接串。
DATABASE_URL = os.getenv("DATABASE_URL", "")

config_logger.debug("数据库配置已从环境变量读取" if DATABASE_URL else "DATABASE_URL 未配置")
...
class Settings(BaseSettings):
    ...
    HERMES_IMAGE_BASE_URL: Optional[str] = None
    HERMES_IMAGE_API_KEY: Optional[str] = None
    HERMES_IMAGE_MODEL: str = "grok-imagine-1.0"
    ...
    TENCENT_COS_DOMAIN: Optional[str] = None
    TENCENT_COS_CDN_DOMAIN: Optional[str] = None
    TENCENT_COS_PREFIX: Optional[str] = None
    ...
    # 回调地址通过环境变量配置；不要提交真实域名、IP 或密钥。
    LINUXDO_REDIRECT_URI: Optional[str] = None
    
    # 前端 URL 配置（用于 OAuth 回调后重定向）
    ...
    # 会话配置
    SESSION_EXPIRE_MINUTES: int = 4320  # 会话过期时间（分钟），默认3天
    ...
    # 提示词工坊配置
    WORKSHOP_MODE: str = "server"  # server: 本地工坊, client: 连接外部工坊服务
    WORKSHOP_CLOUD_URL: str = ""  # client 模式下的外部工坊服务地址
