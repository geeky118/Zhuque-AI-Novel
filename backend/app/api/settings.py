...
async def get_user_ai_settings(
    ...
    *,
    ...
    create_if_missing=True 时，仅用于首次进入设置页的初始化；
    运行时调用建议传 False，避免把环境变量当成真实 AI 配置。
    """
    ...
    if not create_if_missing:
        raise RuntimeError("请先在设置页保存 AI 配置")
...
async def get_user_ai_service(
    ...
    settings = await get_user_ai_settings(db, user.user_id, create_if_missing=True)
...
async def get_user_ai_service_by_user_id(user_id: str, db: AsyncSession) -> AIService:
    """根据 user_id 获取 AI 服务实例（供后台任务使用，无需 Request 上下文）"""
...
async def test_system_smtp_settings(
    ...
    subject = "朱雀AI小说 SMTP 测试邮件"
    text_body = (
        "这是一封来自 朱雀AI小说 系统设置页面的 SMTP 测试邮件。\n\n"
    ...
    <div style=\"font-family: Arial, sans-serif; line-height: 1.7; color: #1f1f1f;\">
      <h2 style=\"margin-bottom: 12px;\">朱雀AI小说 SMTP 测试邮件</h2>
