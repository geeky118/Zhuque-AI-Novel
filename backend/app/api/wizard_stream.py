...
from app.services.comic_style import build_comic_style_instruction, normalize_comic_style
...
async def world_building_generator(
    ...
    try:
        ...
        comic_style = normalize_comic_style(data.get("comic_style"))
        comic_style_prompt = data.get("comic_style_prompt")
        ...
        project = Project(
            ...
            comic_style_prompt=comic_style_prompt,
        ...
        yield await tracker.result({
            ...
            "comic_style": project.comic_style,
            "comic_style_prompt": project.comic_style_prompt,
        })
...
async def characters_generator(
    ...
    try:
            ...
            while retry_count < MAX_RETRIES and not batch_success:
                try:
                    ...
                    base_prompt = (
                        ...
                        "角色、组织、visual_bible、外观描述和训练描述必须服务于该统一漫画风格，"
                        "保证后续角色形象图、角色圣经多视角图和漫画分镜的画风一致。"
                    )
        ...
        # 第六阶段：为缺少 visual_bible 的非组织角色补充生成
        vb_generated = 0
        chars_need_vb = [c for c, _ in created_characters if not c.is_organization and not c.visual_bible]
        if chars_need_vb:
            yield await tracker.saving(f"生成角色视觉圣经（{len(chars_need_vb)}个）...", 0.95)
            from app.api.characters import _generate_visual_bible
            for char in chars_need_vb:
                vb = await _generate_visual_bible(char, user_ai_service, user_id, db)
                if vb:
                    char.visual_bible = vb
                    vb_generated += 1
            await db.flush()
            logger.info(f"✅ 视觉圣经补充生成完成：{vb_generated}/{len(chars_need_vb)}")
