...
from app.api.settings import get_user_ai_service, get_user_ai_service_by_user_id
from app.api.common import verify_project_access

router = APIRouter(prefix="/characters", tags=["角色管理"])
logger = get_logger(__name__)

TRANSIENT_AI_ERROR_HINTS = (
    "429",
    "502",
    "503",
    "504",
...
def _is_transient_ai_error(exc: Exception) -> bool:
    ...
    return any(hint in detail for hint in TRANSIENT_AI_ERROR_HINTS)


async def _generate_visual_bible(
    character: Character,
    ...
    *,
    ...
    """独立调用 AI 为角色生成 visual_bible，失败返回 None 不影响主流程"""
    if character.is_organization:
        if raise_on_error:
            raise ValueError("组织角色不支持视觉圣经")
        return None
    try:
        template = await PromptService.get_template("VISUAL_BIBLE_GENERATION", user_id, db)
        prompt = PromptService.format_prompt(
            template,
            character_name=character.name or "未命名",
            gender=character.gender or "未知",
            age=character.age or "未知",
            appearance=character.appearance or "暂无",
            personality=character.personality or "暂无",
        )
        project = await db.get(Project, character.project_id)
        if project:
            prompt = (
                f"{prompt.strip()}\n\n"
                f"【统一漫画风格】{build_comic_style_instruction(project.comic_style, project.comic_style_prompt)}\n"
                "visual_bible 中的五官比例、线条倾向、服装细节、色彩基调和训练描述必须服务于该统一漫画风格，"
                "后续角色形象图、角色圣经多视角图和漫画分镜会沿用同一风格。"
            )
        max_attempts = 3
        for attempt in range(1, max_attempts + 1):
            try:
                vb = await user_ai_service.call_with_json_retry(
                    prompt=prompt,
                    auto_mcp=False,
                ...
                if attempt >= max_attempts or not _is_transient_ai_error(exc):
                    raise
                wait_seconds = 2 ** attempt
                logger.warning(
                    "visual_bible 生成遇到临时 AI 上游错误，准备重试: character_id=%s attempt=%s/%s wait=%ss error=%s",
                    character.id,
                    attempt,
                    max_attempts,
                    wait_seconds,
        ...
        if isinstance(vb, dict) and vb.get("trigger_token"):
            logger.info("visual_bible 独立生成成功: character_id=%s trigger_token=%s", character.id, vb.get("trigger_token"))
            return vb
        preview = json.dumps(vb, ensure_ascii=False)[:300] if isinstance(vb, dict) else str(vb)[:300]
        message = f"visual_bible 返回格式不正确或缺少 trigger_token: {preview}"
        logger.warning("visual_bible 独立生成返回格式不正确: character_id=%s detail=%s", character.id, message)
        if raise_on_error:
            raise ValueError(message)
        return None
    except Exception as exc:
        logger.warning("visual_bible 独立生成失败，跳过: character_id=%s error=%s", character.id, exc, exc_info=True)
        if raise_on_error:
            raise
        return None
...
async def get_characters(
    ...
    for char in characters:
        ...
        char_dict = {
            ...
            "visual_bible": char.visual_bible
        }
...
async def get_character(
    ...
    char_dict = {
        ...
        "visual_bible": character.visual_bible
    }
...
- 统一漫画风格：{build_comic_style_instruction(project.comic_style, project.comic_style_prompt)}
{existing_chars_info}
{careers_info}
"""
...
"""
            ...
            if not is_organization:
                vb = character_data.get("visual_bible")
                if isinstance(vb, dict) and vb.get("trigger_token"):
                    visual_bible = vb
                    logger.info(f"✅ 角色视觉圣经已提取: trigger_token={vb.get('trigger_token')}")
            ...
            # 如果 AI 没返回 visual_bible，独立调用补充生成
            if not visual_bible and not is_organization:
                yield await tracker.saving("生成角色视觉圣经...", 0.8)
                user_id = getattr(http_request.state, 'user_id', None) or project.user_id
                vb = await _generate_visual_bible(character, user_ai_service, user_id, db)
                if vb:
                    character.visual_bible = vb
                    await db.flush()
                    logger.info(f"✅ 视觉圣经补充生成成功：{character.name}")
...
BIBLE_BATCH_STATE: dict[str, dict] = {}


async def _batch_update_visual_bible(project_id: str, user_id: str, overwrite: bool):
    """后台任务：为项目中所有缺少 visual_bible 的角色批量生成"""
    ...
    async with db_factory() as db:
        try:
            from app.api.settings import get_user_ai_service_by_user_id
            user_ai_service = await get_user_ai_service_by_user_id(user_id, db)
            ...
            for char in characters:
                try:
                    vb = await _generate_visual_bible(char, user_ai_service, user_id, db)
                    if vb:
                        char.visual_bible = vb
            ...
            logger.error("批量更新 visual_bible 异常: project_id=%s", project_id, exc_info=True)
            task_key = f"bible_batch:{project_id}"
...
async def batch_update_visual_bible(
    project_id: str,
    ...
    task_key = f"bible_batch:{project_id}"
    if task_key in BIBLE_BATCH_STATE and BIBLE_BATCH_STATE[task_key].get("status") == "generating":
        raise HTTPException(status_code=409, detail="正在执行中，请等待完成")

    # 统计需要更新的角色数
    query = select(func.count(Character.id)).where(
        Character.project_id == project_id,
        Character.is_organization == False,
    )
    if not overwrite:
    ...
    if total == 0:
        return {"status": "no_action", "total": 0, "message": "没有需要更新的角色"}

    background_tasks.add_task(
        _batch_update_visual_bible,
        project_id,
        user_id,
        overwrite,
    )
...
@router.get("/projects/{project_id}/visual-bible/batch-status", summary="查询批量视觉圣经生成状态")
async def get_batch_visual_bible_status(
    project_id: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    ...
    task_key = f"bible_batch:{project_id}"
    state = BIBLE_BATCH_STATE.get(task_key)
    if not state:
        return {"status": "idle"}
    return state


@router.post("/{character_id}/visual-bible/regenerate", summary="为单个角色重新生成视觉圣经")
async def regenerate_visual_bible(
    character_id: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    ...
    await verify_project_access(character.project_id, user_id, db)
    if character.is_organization:
        raise HTTPException(status_code=400, detail="组织角色不支持视觉圣经")

    user_ai_service = await get_user_ai_service_by_user_id(user_id, db)
    try:
        vb = await _generate_visual_bible(character, user_ai_service, user_id, db, raise_on_error=True)
    except HTTPException:
        raise
    except Exception as exc:
        logger.error("单角色 visual_bible 重新生成失败: character_id=%s", character_id, exc_info=True)
        detail = str(exc).strip() or "未知错误"
        raise HTTPException(status_code=502, detail=f"视觉圣经生成失败: {detail[:500]}") from exc
    if not vb:
        raise HTTPException(status_code=502, detail="视觉圣经生成失败：AI 未返回有效 visual_bible")

    character.visual_bible = vb
    await db.commit()
    return {"status": "ok", "visual_bible": vb}
