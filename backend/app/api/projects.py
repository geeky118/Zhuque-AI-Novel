...
from app.services.comic_style import (
    DEFAULT_COMIC_STYLE,
    comic_style_options_payload,
    normalize_comic_style,
)
...
async def create_project(
    ...
    try:
        ...
        project_data['comic_style'] = normalize_comic_style(project_data.get('comic_style'))
...
async def update_project(
    ...
    try:
        ...
        if 'comic_style' in update_data:
            update_data['comic_style'] = normalize_comic_style(update_data.get('comic_style'))
...
@router.get("/comic-style/options", summary="获取漫画风格预设")
async def get_comic_style_options():
    return {
        "default": DEFAULT_COMIC_STYLE,
        "items": comic_style_options_payload(),
    }
