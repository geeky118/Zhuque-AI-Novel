...
active_batch_generation_tasks: set[str] = set()
...
def _recover_stale_analysis_task(task: Optional[AnalysisTask], current_time: datetime) -> bool:
    """将卡住的分析任务恢复为失败态，返回是否发生恢复。"""
    if not task:
        return False
...
async def get_analysis_task_status(
    ...
    auto_recovered = _recover_stale_analysis_task(task, current_time)
    if auto_recovered:
        await db.commit()
        await db.refresh(task)
        logger.warning(f"🔄 自动恢复卡住的分析任务: {task.id}, 章节: {chapter_id}")
...
async def get_project_analysis_task_statuses(
    ...
    for chapter_id in chapter_ids:
        ...
        auto_recovered = _recover_stale_analysis_task(task, current_time)
        if auto_recovered and task:
            recovered_task_ids.append(task.id)
        items[chapter_id] = _build_analysis_task_status_payload(chapter_id, task, auto_recovered)

    if recovered_task_ids:
        await db.commit()
        logger.warning(
            f"🔄 批量状态查询自动恢复卡住的分析任务: {len(recovered_task_ids)}个 "
...
async def batch_generate_chapters_in_order(
    ...
    active_batch_generation_tasks.add(batch_id)
...
async def get_active_batch_generation(
    ...
    if not active_batch_generation_tasks:
        return {
    ...
    result = await db.execute(
        ...
        .where(BatchGenerationTask.id.in_(list(active_batch_generation_tasks)))
...
async def execute_batch_generation_in_order(
    ...
    try:
        ...
        active_batch_generation_tasks.discard(batch_id)
        if db_session:
            await db_session.close()
...
确保在整个章节创作过程中始终保持风格的一致性。"""
    ...
    # 批量生成不需要 SSE 流式返回，改用普通请求可减少长连接中断概率
