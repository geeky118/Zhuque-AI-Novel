...
_batch_outline_expansion_tasks: dict[str, dict[str, Any]] = {}
...
def _update_batch_outline_task(task_id: str, **updates: Any) -> None:
    task = _batch_outline_expansion_tasks.get(task_id)
    if not task:
        return
    task.update(updates)
...
async def _outline_has_expanded_chapters(db: AsyncSession, outline: Outline) -> int:
    result = await db.execute(
...
async def _execute_batch_outline_expansion_task(
    *,
    task_id: str,
    user_id: str,
    ...
    chapters_per_outline = int(request_data.get("chapters_per_outline", 3))
    expansion_strategy = request_data.get("expansion_strategy", "balanced")

    engine = await get_engine(user_id)
    AsyncSessionLocal = async_sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)

    _update_batch_outline_task(
        task_id,
        status="running",
        progress=1,
        message="后台批量展开任务已启动",
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            ai_service = await get_user_ai_service_by_user_id(user_id, db)
            ...
            _update_batch_outline_task(task_id, total=total, message=f"共找到 {total} 个大纲，开始后台展开")
            ...
            for idx, outline in enumerate(outlines):
                base_progress = int((idx / total) * 95)
                _update_batch_outline_task(
                    task_id,
                    current_outline_id=outline.id,
                    current_outline_title=outline.title,
                    progress=max(1, base_progress),
                    message=f"处理第 {idx + 1}/{total} 个大纲：{outline.title}",
                )

                existing_count = await _outline_has_expanded_chapters(db, outline)
                if existing_count > 0:
                    skipped_item = {
                        "outline_id": outline.id,
                        "outline_title": outline.title,
                        "reason": f"已展开 {existing_count} 章",
                    }
                    skipped.append(skipped_item)
                    _update_batch_outline_task(
                        task_id,
                        skipped_count=len(skipped),
                        message=f"跳过已展开大纲：{outline.title}",
                    )
                    continue
                ...
                try:
                    chapter_plans = await expansion_service.analyze_outline_for_chapters(
                        ...
                        target_chapter_count=chapters_per_outline,
                        expansion_strategy=expansion_strategy,
                        enable_scene_analysis=request_data.get("enable_scene_analysis", True),
                    ...
                    if existing_count > 0:
                        skipped.append({
                            "outline_id": outline.id,
                            "outline_title": outline.title,
                            "reason": f"生成规划期间已被展开 {existing_count} 章",
                        })
                        continue
                    ...
                    _update_batch_outline_task(
                        ...
                        total_chapters_created=created_total,
                        progress=int(((idx + 1) / total) * 95),
                        message=f"{outline.title} 已创建 {len(chapters)} 个章节",
                    )
                except Exception as exc:
                    logger.error("后台批量展开大纲失败: task=%s outline=%s error=%s", task_id, outline.id, exc, exc_info=True)
                    failed.append({
                        "outline_id": outline.id,
                        "outline_title": outline.title,
                        "error": str(exc),
                    ...
                    _update_batch_outline_task(
                        ...
                        message=f"{outline.title} 展开失败，继续处理后续大纲",
                    )
            ...
            _update_batch_outline_task(
                ...
                status=status,
                progress=100,
                message=f"批量展开完成：创建 {created_total} 章，跳过 {len(skipped)} 个，失败 {len(failed)} 个",
                completed=len(results),
                skipped_count=len(skipped),
                failed_count=len(failed),
                total_chapters_created=created_total,
                skipped_outlines=skipped,
                failed_outlines=failed,
                expansion_results=results,
                completed_at=datetime.utcnow().isoformat(),
            )
        except Exception as exc:
            logger.error("后台批量展开任务失败: task=%s error=%s", task_id, exc, exc_info=True)
            ...
            _update_batch_outline_task(
                ...
                status="failed",
                progress=100,
                message=f"批量展开失败：{exc}",
                error=str(exc),
                completed_at=datetime.utcnow().isoformat(),
            )


@router.post("/batch-expand-tasks", summary="创建后台批量展开大纲任务")
async def create_batch_outline_expansion_task(
    data: Dict[str, Any],
    request: Request,
    ...
    running_task = next(
        (
            task for task in _batch_outline_expansion_tasks.values()
            if task.get("project_id") == project_id and task.get("user_id") == user_id and task.get("status") in {"pending", "running"}
        ),
        None,
    )
    if running_task:
        return {
            ...
            "message": "已有批量展开任务正在运行",
        }

    task_id = str(uuid.uuid4())
    now = datetime.utcnow().isoformat()
    _batch_outline_expansion_tasks[task_id] = {
        "task_id": task_id,
        "project_id": project_id,
        "user_id": user_id,
        "status": "pending",
    ...
    request_data["auto_create_chapters"] = True
    background_tasks.add_task(
        _execute_batch_outline_expansion_task,
        task_id=task_id,
        user_id=user_id,
    ...
    return {
        "task_id": task_id,
        "status": "pending",
        "message": "批量展开任务已进入后台队列",
    }


@router.get("/batch-expand-tasks/{task_id}", summary="查询后台批量展开大纲任务状态")
async def get_batch_outline_expansion_task_status(
    task_id: str,
    request: Request,
):
    user_id = getattr(request.state, "user_id", None)
    ...
    if not task or task.get("user_id") != user_id:
        raise HTTPException(status_code=404, detail="批量展开任务不存在")
    return task
...
async def create_chapters_from_existing_plans(
    ...
    try:
        existing_count = await _outline_has_expanded_chapters(db, outline)
        if existing_count > 0:
            raise HTTPException(status_code=400, detail=f"该大纲已展开 {existing_count} 个章节，不能重复创建")
