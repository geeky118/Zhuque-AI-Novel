...
async def create_book_import_task(
    request: Request,
    file: UploadFile | None = File(default=None, description="TXT 文件"),
    ...
    extract_mode: str = Form(default="full", description="解析范围：tail=截取末章，full=整本"),
...
@router.get("/projects/{project_id}/followup-status", summary="查询拆书导入后的后台补全状态")
async def get_post_import_followup_status(
    project_id: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    """查询导入后补全状态，包括角色、组织、关系、职业、记忆、伏笔和章节分析计数。"""
    user_id = getattr(request.state, "user_id", None)
    if not user_id:
        raise HTTPException(status_code=401, detail="未登录")

    return await book_import_service.get_post_import_followup_status(
...
@router.post("/projects/{project_id}/followup/resume", summary="重新启动拆书导入后的项目补全")
async def resume_post_import_followup(
    project_id: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    """只重启世界观、职业、角色、组织和关系补全，不额外启动整本章节分析队列。"""
    user_id = getattr(request.state, "user_id", None)
    if not user_id:
        raise HTTPException(status_code=401, detail="未登录")

    return await book_import_service.resume_post_import_followup(
        project_id=project_id,
        user_id=user_id,
        db=db,
    )
...
@router.post("/projects/{project_id}/resume", summary="重新启动拆书导入后的后台补全")
async def resume_post_import_pipeline(
    project_id: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    """当基础导入已落库但后续补全未完成时，重新启动后台任务。"""
    user_id = getattr(request.state, "user_id", None)
    if not user_id:
        raise HTTPException(status_code=401, detail="未登录")

    return await book_import_service.resume_post_import_pipeline(
        project_id=project_id,
        user_id=user_id,
        db=db,
    )
