...
def _is_transient_analysis_error(error_reason: str | None) -> bool:
    if not error_reason:
        return False
    detail = error_reason.lower()
    return any(hint in detail for hint in TRANSIENT_ANALYSIS_AI_HINTS)


def _analysis_retry_wait_time(attempt: int, error_reason: str | None) -> int:
    if _is_transient_analysis_error(error_reason):
        base = 60 if error_reason and ("429" in error_reason or "too many requests" in error_reason.lower()) else 20
        return min(base * (2 ** max(attempt - 1, 0)), 600)
            ...
            try:
                ...
                if not accumulated_text or len(accumulated_text.strip()) < 10:
                    ...
                    if attempt < max_retries:
                        wait_time = _analysis_retry_wait_time(attempt, last_error)
