...
{{
  ...
  "visual_bible": {{
    "trigger_token": "角色名拼音或英文",
    "immutable_traits": {{
      "hair_color": "发色",
      "eye_color": "瞳色",
      "hairstyle": "发型描述",
      "body_type": "体型描述",
      "signature_accessories": "标志性配饰",
      "temperament": "气质特征"
    }},
    "forbidden_traits": ["不应出现的特征"],
    "views": [
      {{ "angle": "front_full", "description": "正面全身描述" }},
      {{ "angle": "front_portrait", "description": "正面半身描述" }},
      {{ "angle": "three_quarter", "description": "四分之三侧身描述" }},
      {{ "angle": "side", "description": "纯侧面描述" }},
      {{ "angle": "back", "description": "背面全身描述" }}
    ],
    "expressions": [
      {{ "name": "平静", "description": "面无表情描述" }},
      {{ "name": "微笑", "description": "微笑描述" }},
      {{ "name": "愤怒", "description": "愤怒描述" }},
      {{ "name": "惊讶", "description": "惊讶描述" }}
    ],
    "outfits": [
      {{ "name": "默认", "description": "日常着装描述" }}
    ],
    "training_caption": "trigger_token, 角色核心外观描述"
  }}
}}
...
✅ 关系网络：角色间形成合理关系
✅ 组织合理：组织是推动剧情的关键力量
✅ 每个角色（非组织）必须包含 visual_bible，包含完整的 trigger_token、immutable_traits、views（5个）、expressions（4个）、outfits、training_caption
...
{{
  ...
  "visual_bible": {{
    "trigger_token": "角色名拼音或英文（如 li_ming，用作 LoRA 训练触发词）",
    "immutable_traits": {{
      "hair_color": "发色",
      "eye_color": "瞳色",
      "hairstyle": "发型描述",
    ...
    "forbidden_traits": ["不应出现在该角色身上的特征1", "特征2", "特征3"],
    "views": [
      {{ "angle": "front_full", "description": "正面全身，站姿描述" }},
      {{ "angle": "front_portrait", "description": "正面半身，胸部以上" }},
      {{ "angle": "three_quarter", "description": "四分之三侧身，姿态描述" }},
      {{ "angle": "side", "description": "纯侧面，展示发型轮廓" }},
      {{ "angle": "back", "description": "背面全身，展示背影" }}
    ],
    "expressions": [
      {{ "name": "平静", "description": "面无表情，眼神描述" }},
      {{ "name": "微笑", "description": "微笑时的面部特征" }},
      {{ "name": "愤怒", "description": "愤怒时的面部特征" }},
      {{ "name": "惊讶", "description": "惊讶时的面部特征" }}
    ],
    "outfits": [
      {{ "name": "默认", "description": "日常着装详细描述" }},
      {{ "name": "特殊场景", "description": "特殊场合着装描述" }}
    ],
    "training_caption": "trigger_token, 角色核心外观的自然语言描述，包含发色、瞳色、体型、标志性特征"
  }}
}}
...
✅ 为故事服务：设定要推动剧情发展
✅ 职业匹配：职业选择与角色高度契合
✅ 视觉圣经：visual_bible必须包含完整的多视角描述、表情、服装、不可变特征和禁止特征
...
❌ 脸谱化的角色设定
    ...
    VISUAL_BIBLE_GENERATION = """<system>
你是专业的角色视觉设计师，擅长为小说角色创建详细的视觉圣经（Character Visual Bible），用于 AI 绘图和 LoRA 训练。
</system>

<task>
根据以下角色信息，生成角色视觉圣经 JSON。视觉圣经用于 AI 绘图一致性保障和 LoRA 模型训练。
</task>
...
{{
  ...
  "expressions": [
    {{ "name": "平静", "description": "面无表情时的面部描述" }},
    {{ "name": "微笑", "description": "微笑时的面部特征" }},
    {{ "name": "愤怒", "description": "愤怒时的面部特征" }},
    {{ "name": "惊讶", "description": "惊讶时的面部特征" }}
  ],
...
<constraints>
✅ trigger_token 使用角色名拼音或英文简称
✅ immutable_traits 必须从外观描述中提取，不可臆造
✅ views 必须包含 5 个标准视角
✅ expressions 至少 4 个
✅ outfits 至少 1 个
✅ training_caption 以 trigger_token 开头
✅ 纯JSON输出，无markdown标记
❌ 不要编造外观描述中不存在的特征
</constraints>"""
...
{{
  ...
  "visual_bible": {{
    "trigger_token": "角色名拼音或英文（如 li_ming）",
    "immutable_traits": {{
      "hair_color": "发色",
      "eye_color": "瞳色",
      "hairstyle": "发型描述",
...
✅ 职业选择必须从可用列表中选择
✅ visual_bible必须包含完整的 trigger_token、immutable_traits、views（5个）、expressions（4个）、outfits、training_caption
...
❌ 偏离用户的修改要求
    ...
    STORYBOARD_GENERATION = """<system>
你是一位专业的漫画分镜脚本编剧，擅长将小说章节内容转化为结构化、可执行、可落地的漫画分镜脚本。
</system>

<task>
【分镜脚本生成任务】
将小说《{project_title}》第{chapter_number}章《{chapter_title}》的正文内容，结合分镜规划JSON，转化为最终可直接用于绘图的漫画分镜脚本。

【基本要求】
- 小说类型：{genre}
- 目标页数：{target_pages}页（每页2-4个分镜格）
- 输出格式：严格按照指定的JSON格式输出
</task>

<chapter_content priority="P0">
【章节正文】
{chapter_content}
</chapter_content>
...
<bridge_context priority="P0">
【章节桥接信息】
{bridge_context}
</bridge_context>

<continuity_pack priority="P0">
【连续性包】
{continuity_pack}
</continuity_pack>

<storyboard_plan priority="P0">
【分镜规划JSON】
{storyboard_plan}
</storyboard_plan>

<output_format priority="P0">
【输出格式要求】
  ...
  "pages": [
    {{
      ...
      "panels": [
        {{
          "panel_number": 1,
          "description": "画面描述（用于图片生成的详细视觉描述）",
          "dialogue": "对话内容（如有）",
          "scene": "场景/背景描述",
          "characters": ["出场角色名"],
          "camera_angle": "镜头角度（远景/中景/近景/特写）",
          "emotion": "情绪氛围"
        }}
      ]
    }}
...
<style_rules>
【视觉风格要求】
- 画面应保持统一的商业漫画可读性，角色造型、服装、发型、镜头语言和色彩要与项目统一风格一致
- 亲密或成人向情节只保留非露骨的情绪张力、距离感和镜头调度，不要描写裸露、性行为或身体私密部位
- 画面描述使用简体中文完整表达，突出镜头、动作、场景与情绪，不要依赖外语关键词堆叠
</style_rules>

<constraints>
【必须遵守】
✅ 保留原文核心剧情和对话，包括亲密/情爱场景
✅ 每页分镜数量控制在2-4格
✅ 画面描述要具体、可视化，适合图片生成
✅ 镜头角度要有变化，避免单调
✅ 角色名称必须与提供的角色信息一致
✅ 对话精简，保留关键台词
✅ 场景转换要清晰标注
✅ 成人向情节只保留含蓄的视觉表达，不要输出露骨内容
✅ 邻章桥接信息优先级高于一般氛围描述，必须自然承接上一章收束并为下一章留出转场空间
...
❌ 不要完全删除或跳过情爱相关内容
    ...
    STORYBOARD_PLANNING = """<system>
你是一位专业的漫画分镜总监，擅长先把小说章节拆成页面节奏、场景转场和镜头节拍，再交给分镜编剧执行。
</system>

<task>
【分镜规划任务】
将小说《{project_title}》第{chapter_number}章《{chapter_title}》的正文内容，规划为{target_pages}页漫画分镜草案。
...
<style_rules priority="P1">
【统一风格】
{comic_style_instruction}
</style_rules>
...
```json
{{
  "chapter_summary": "本章一句话概括",
  "continuity_rules": ["连续性约束1", "连续性约束2"],
  "pages": [
    {{
      "page_number": 1,
      "page_goal": "本页叙事目标",
      "scene": "场景/背景概述",
      "turning_point": "本页转折或钩子",
      "panel_count": 3,
      "must_keep": ["必须保留的剧情点1", "必须保留的剧情点2"],
      "panel_plan": [
        {{
          "panel_number": 1,
          "beat": "镜头节拍或剧情动作",
          "visual_goal": "这一格的画面目标",
          "camera_intent": "镜头意图",
          "emotion": "情绪氛围",
          "dialogue_focus": "对话重点（如无则留空）"
        }}
      ]
    }}
  ]
...
<constraints>
【必须遵守】
✅ 页数必须严格等于{target_pages}页
✅ 每页 panel_count 必须在2-4之间
✅ 规划要覆盖章节主线、转折和收束
✅ 角色名必须来自提供的角色信息或章节正文
✅ 邻章桥接信息优先级高于一般氛围描述，必须自然承接上一章收束并为下一章留出转场空间
✅ 输出必须可被JSON直接解析
...
❌ 漏掉主要转折或情绪节点
    ...
    STORYBOARD_REPAIR = """<system>
你是一位专业的漫画分镜脚本修订编辑，擅长在不改变剧情主干的前提下修复JSON结构、补齐缺失字段并统一页码与格号。
</system>

<task>
【修复任务】
根据原始章节内容、分镜规划JSON、当前分镜JSON和校验问题，输出修复后的最终漫画分镜脚本。

【基本要求】
- 保留原有剧情节拍与关键节点
- 只修复结构、字段、页码、格号、缺失值和明显的不一致
- 仍需严格输出JSON
</task>
...
<current_storyboard priority="P0">
【当前分镜JSON】
{current_storyboard}
</current_storyboard>

<validation_issues priority="P0">
【校验问题】
{validation_issues}
</validation_issues>
...
<output_format priority="P0">
【输出格式要求】
请严格输出与最终分镜脚本一致的JSON结构，不要包含任何其他文字。
</output_format>

<constraints>
【必须遵守】
✅ 只修复错误，不要重写整章剧情
✅ 页码从1开始递增，格号在每页内从1开始递增
✅ 每页分镜数量控制在2-4格
✅ 对话保持精简，画面描述保持可视化
✅ 邻章桥接信息优先级高于一般氛围描述，修复时不得破坏章节之间的连续衔接
✅ 输出必须可被JSON直接解析
...
❌ 使用 markdown 或代码块
    ...
    # 拆书导入-深度故事档案抽取
    BOOK_IMPORT_DEEP_STORY_DOSSIER = """<system>
你是小说拆书智能体编排中的“故事档案分析员”，擅长从已完成正文中抽取可落库的人物、关系、组织、大纲记忆和伏笔。
</system>

<task>
【任务】
分析给定章节批次，抽取真实出现在正文中的故事档案。

【目标】
- 只基于正文证据抽取，不补写、不续写、不虚构。
- 输出可直接入库的结构化数据，供系统创建角色、组织、关系、记忆和伏笔。
- 允许慢而细：优先准确、具体、可复核。
- 所有实体都必须能在本批次正文里找到名字或明确证据；证据不足时宁可不返回。
</task>
...
{{
  "source_platform_notes": ["平台导出噪音、章节异常、版权页或广告等观察；没有则返回空数组"],
  "world_notes": ["本批次出现的世界规则、地点、时代、势力格局、特殊设定"],
  "characters": [
    {{
      "name": "角色名",
      "role_type": "protagonist/supporting/antagonist",
      "gender": "性别或空",
      "age": "年龄或空",
      "personality": "基于正文的性格与行为模式，80-180字",
      "background": "已知背景、身份、经历，80-220字",
      "appearance": "外貌或标志性特征，没有则空",
      "current_state": "当前状态、处境或心理变化，60-160字",
      "traits": ["标签1", "标签2"],
  ...
  "relationships": [
    {{
      ...
      "relationship_type": "关系类型",
      "intimacy_level": 50,
      "status": "active/past/broken/complicated",
      "description": "关系证据与变化，60-180字"
    }}
  ],
  "organizations": [
    {{
      "name": "组织或势力名",
      "organization_type": "门派/公司/家族/军队/政府/帮派/神秘组织等",
      "purpose": "组织目标或立场",
      "background": "组织来历、运作方式、剧情作用",
      "location": "所在地或活动范围",
      "power_level": 70,
      "members": [
        {{"name": "角色名", "position": "职位或身份", "status": "active", "loyalty": 60}}
      ],
      "traits": ["特征1", "特征2"]
    }}
  ],
  "memories": [
    {{
      "title": "记忆标题",
      "content": "关键事件、角色状态变化、世界观细节或剧情转折，80-260字",
      "memory_type": "plot_point/character_event/world_detail/hook/foreshadow/dialogue/scene",
      "chapter_number": 1,
      "related_characters": ["角色名"],
      "related_locations": ["地点名"],
      "tags": ["标签"],
  ...
  "foreshadows": [
    {{
      "title": "伏笔标题",
      "content": "伏笔或悬念的具体内容与正文证据",
      "plant_chapter_number": 1,
      "target_resolve_chapter_number": null,
      "status": "planted/resolved/partially_resolved",
      "category": "identity/mystery/item/relationship/event",
      "related_characters": ["角色名"],
      "tags": ["标签"],
      "importance": 0.7,
      "strength": 7,
...
<constraints>
【必须遵守】
✅ 所有人名、组织名必须来自正文或章节标题中的明确信息
✅ relationships 只能引用本批次或已明确出现的人物，description 必须写出证据章节或原文事实
✅ organizations 只能返回正文明确出现的势力/机构/家族/门派/公司等，不能把地点、村庄、学校、普通群体误当组织
✅ 角色定位、性格、背景、当前状态必须来自行为或叙述证据，不能使用通用人设模板
✅ memories 每章至少抽取1条，重要章节可多条
✅ 伏笔不确定时宁缺毋滥
✅ 数值字段必须为数字，空值使用 null
✅ 输出必须可被JSON直接解析
...
❌ 把没有证据的推测写成事实
❌ 使用“某人”“主角”等泛称作为角色名
❌ 为了补齐角色、组织或关系数量而创造正文没有出现的名字
❌ 根据类型套路添加未在正文出现的门派、职业、反派、导师、同伴
</constraints>"""
...
""")
    ...
    def get_all_system_templates(cls) -> list:
        ...
        template_definitions = {
            ...
            "BOOK_IMPORT_DEEP_STORY_DOSSIER": {
                "name": "拆书导入-深度故事档案",
                "category": "拆书导入",
                "description": "从TXT正文分批抽取人物、关系、组织、记忆和伏笔",
                "parameters": [
                    "title", "genre", "theme", "narrative_perspective",
                    "start_chapter", "end_chapter", "chapters_text"
                ]
            },
            ...
            "STORYBOARD_GENERATION": {
                "name": "分镜脚本生成",
                "category": "漫画分镜",
                "description": "将小说章节内容转化为结构化的漫画分镜脚本（JSON格式）",
                "parameters": ["project_title", "genre", "chapter_number", "chapter_title",
                             "chapter_content", "characters_info", "target_pages", "storyboard_plan",
                             "bridge_context", "continuity_pack", "comic_style_instruction"]
            },
            "STORYBOARD_PLANNING": {
                "name": "分镜脚本规划",
                "category": "漫画分镜",
                "description": "先把小说章节拆成页级节奏、场景转场和镜头节拍的规划JSON",
                "parameters": ["project_title", "genre", "chapter_number", "chapter_title",
                             "chapter_content", "characters_info", "target_pages", "bridge_context", "continuity_pack",
                             "comic_style_instruction"]
            },
            "STORYBOARD_REPAIR": {
                "name": "分镜脚本修复",
                "category": "漫画分镜",
                "description": "基于校验问题修复分镜JSON结构并统一页码与格号",
                "parameters": ["chapter_content", "characters_info", "storyboard_plan",
                             "current_storyboard", "validation_issues", "bridge_context", "continuity_pack"]
            },
            "PLOT_ANALYSIS": {
                "name": "情节分析",
                "category": "情节分析",
