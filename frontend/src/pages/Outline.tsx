...
import type { OutlineExpansionResponse, BatchOutlineExpansionResponse, BatchOutlineExpansionTaskStatus, ChapterPlanItem, ApiError, Character } from '../types';
...
export default function Outline() {
  ...
  const handleGenerate = async (values: GenerateFormValues) => {
    try {
      ...
      // 使用SSE客户端
      const apiUrl = buildApiPath('/outlines/generate-stream');
      ...
      modalApi.confirm({
        ...
        onOk: async () => {
          try {
            ...
            // 使用SSE客户端调用新的流式端点
            const apiUrl = buildApiPath(`/outlines/${outlineId}/expand-stream`);
            const client = new SSEPostClient(apiUrl, requestData, {
              onProgress: (msg: string, progress: number) => {
                setSSEMessage(msg);
                setSSEProgress(progress);
  ...
  const pollBatchExpandTask = (taskId: string) => {
    const poll = async () => {
      try {
        const status: BatchOutlineExpansionTaskStatus = await outlineApi.getBatchExpandTaskStatus(taskId);
        setSSEProgress(Math.max(0, Math.min(100, status.progress || 0)));
        setSSEMessage(status.message || '批量展开任务运行中...');

        if (['completed', 'partial_failed', 'failed'].includes(status.status)) {
          ...
          if (status.status === 'completed') {
            message.success(`批量展开完成：创建 ${status.total_chapters_created} 章，跳过 ${status.skipped_count} 个已展开大纲`);
          } else if (status.status === 'partial_failed') {
            message.warning(`批量展开部分完成：创建 ${status.total_chapters_created} 章，跳过 ${status.skipped_count} 个，失败 ${status.failed_count} 个`);
          } else {
            message.error(status.message || '批量展开失败');
          }
        }
      } catch (error) {
        window.clearInterval(timer);
        setSSEModalVisible(false);
        setIsExpanding(false);
        console.error('查询批量展开任务失败:', error);
        message.error('查询批量展开任务失败');
      }
    };
    ...
    modalApi.confirm({
      ...
      onOk: async () => {
        try {
          ...
          const task = await outlineApi.createBatchExpandTask(requestData);
          setSSEMessage(task.message || '后台任务已创建');
          pollBatchExpandTask(task.task_id);
