...
export default function PromptTemplates() {
  ...
  const loadTemplates = async () => {
    try {
      ...
      setCategories(response as CategoryGroup[]);
    } catch (error: unknown) {
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      await promptTemplateApi.createOrUpdate({
    ...
    modal.confirm({
      ...
      centered: true,
        onOk: async () => {
          try {
            setLoading(true);
            await promptTemplateApi.reset(templateKey);
  ...
  const handleToggleActive = async (template: PromptTemplate, checked: boolean) => {
    try {
      await promptTemplateApi.update(template.template_key, {
        is_active: checked
      });
      loadTemplates();
    } catch (error: unknown) {
  ...
  const handleExport = async () => {
    try {
      const response = await promptTemplateApi.exportAll() as {
        statistics?: {
          total: number;
          customized: number;
  ...
  const handleImport = async (file: File) => {
    try {
      ...
      const response = await promptTemplateApi.importAll(data) as {
        statistics?: {
          kept_system_default: number;
          created_or_updated: number;
          converted_to_custom: number;
        };
        converted_templates?: Array<{ template_key: string; template_name: string }>;
      };
              ...
              <Space wrap style={{ justifyContent: isMobile ? 'flex-start' : 'flex-end', width: '100%' }}>
                <Button
                  ...
                  style={{
                    ...
                    boxShadow: `0 8px 20px ${alphaColor(zhuqueColors.ink, 0.1)}`,
            ...
            description={
              <div>
                ...
                <Text style={{ fontSize: isMobile ? 12 : 13, display: 'block' }}>
                  • <strong>已自定义模板</strong>（朱砂色头部）：可通过开关控制启用/禁用，使用 <Text code>{'{variable_name}'}</Text> 格式表示变量占位符。点击"重置"可恢复为系统默认。
                </Text>
              </div>
            }
            type="info"
