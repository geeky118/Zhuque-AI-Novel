import { Card, Button, Spin, Space, Tag, Typography, Alert, theme, Modal, Input, message } from 'antd';
...
interface BookshelfPageProps {
  ...
  onEditCover: (project: Project, prompt: string) => void | Promise<void>;
...
export default function BookshelfPage({
  ...
  const openCoverEditModal = (event: React.MouseEvent<HTMLElement>, project: Project) => {
    event.stopPropagation();
    setCoverEditProject(project);
    setCoverEditPrompt('');
  };

  const submitCoverEdit = async () => {
    if (!coverEditProject) {
      return;
    }
    const prompt = coverEditPrompt.trim();
    if (!prompt) {
      message.warning('请输入封面改图要求');
      return;
    }
    if (coverGeneratingIds[coverEditProject.id]) {
      return;
    }

    const project = coverEditProject;
    setCoverGeneratingIds((prev) => ({ ...prev, [project.id]: true }));
    try {
      await onEditCover(project, prompt);
      setCoverEditProject(null);
      setCoverEditPrompt('');
    } finally {
      setCoverGeneratingIds((prev) => ({ ...prev, [project.id]: false }));
    }
    ...
    <div>
      <Card
        ...
        style={{
          ...
          background: `linear-gradient(135deg, ${teal} 0%, #356B67 62%, ${alphaColor(cinnabar, 0.82)} 100%)`,
          boxShadow: `0 18px 42px ${alphaColor(ink, isDark ? 0.2 : 0.12)}`,
          border: `1px solid ${alphaColor(gold, 0.18)}`,
          position: 'relative',
          overflow: 'hidden',
        }}
        styles={{ body: { padding: isMobile ? '14px 14px' : '20px 22px' } }}
      >
        <div style={{ position: 'absolute', top: 0, right: 0, width: 260, height: '100%', background: `linear-gradient(120deg, transparent 0%, ${alphaColor(gold, 0.16)} 42%, transparent 74%)`, transform: 'skewX(-18deg)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: -70, left: '24%', width: 220, height: 140, borderTop: `1px solid ${alphaColor(token.colorWhite, 0.18)}`, transform: 'rotate(-8deg)', pointerEvents: 'none' }} />
      ...
      <Spin spinning={loading}>
        <div style={{
          ...
          border: `1px solid ${isDark ? alphaColor(token.colorBorder, 0.42) : alphaColor(cinnabar, 0.08)}`,
          backgroundColor: isDark ? alphaColor(token.colorBgContainer, 0.42) : alphaColor(zhuqueColors.paperSoft, 0.72),
          backgroundImage: `
            radial-gradient(${alphaColor(cinnabar, isDark ? 0.18 : 0.1)} 1px, transparent 0),
            linear-gradient(180deg, ${alphaColor(token.colorWhite, isDark ? 0.02 : 0.42)} 0%, transparent 100%)
          `,
          backgroundSize: '18px 18px, auto',
          boxShadow: `inset 0 1px 0 ${alphaColor(token.colorWhite, isDark ? 0.08 : 0.58)}, 0 18px 42px ${alphaColor(ink, isDark ? 0.18 : 0.08)}`,
            ...
            >
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  display: 'grid',
                  placeItems: 'center',
                  background: alphaColor(token.colorWhite, isDark ? 0.08 : 0.72),
                  border: `1px solid ${alphaColor(gold, isDark ? 0.26 : 0.36)}`,
                  boxShadow: `0 18px 42px ${alphaColor(cinnabar, isDark ? 0.12 : 0.14)}`,
                }}>
                  <img src={zhuqueAssetUrls.brandMark} alt={APP_NAME} style={{ width: isMobile ? 54 : 64, height: isMobile ? 54 : 64, objectFit: 'contain' }} />
                </div>
                ...
                <div style={{ width: '100%', maxWidth: 240, display: 'flex', flexDirection: 'column', gap: isMobile ? 10 : 14 }}>
                  <Button
                    ...
                    style={{
                      ...
                      borderRadius: 10,
                      background: `linear-gradient(135deg, ${cinnabar} 0%, #C24738 100%)`,
                      boxShadow: `0 10px 18px ${alphaColor(cinnabar, isDark ? 0.18 : 0.24)}`,
                      border: 'none',
                    }}
                    block
                  >
                  ...
                  <Button
                    ...
                    style={{
                      ...
                      color: `color-mix(in srgb, ${gold} ${isDark ? 82 : 70}%, ${token.colorText} ${isDark ? 18 : 30}%)`,
                      background: `linear-gradient(180deg, ${alphaColor(gold, isDark ? 0.12 : 0.12)} 0%, ${alphaColor(gold, isDark ? 0.2 : 0.2)} 100%)`,
                    }}
                    block
                  >
                    灵感模式
                          ...
                          <div style={{
                            ...
                            <Space wrap>
                              ...
                              <Button
                                ...
                                onClick={(e) => openCoverEditModal(e, project)}
                                style={{
                                  color: token.colorWhite,
                                  background: 'rgba(255,255,255,0.14)',
                                  borderColor: 'rgba(255,255,255,0.22)',
      ...
      <Modal
        title={coverEditProject ? `修改《${coverEditProject.title}》封面` : '修改封面'}
        open={!!coverEditProject}
        okText="开始改图"
        cancelText="取消"
        confirmLoading={!!coverEditProject && !!coverGeneratingIds[coverEditProject.id]}
        onOk={() => void submitCoverEdit()}
        onCancel={() => {
          setCoverEditProject(null);
          setCoverEditPrompt('');
        }}
      ...
      >
        <TextArea
          ...
          onChange={(event) => setCoverEditPrompt(event.target.value)}
          placeholder="例如：把背景改成雨夜山村，人物服装保持不变，标题仍然清晰"
          rows={5}
          maxLength={3000}
          showCount
        />
