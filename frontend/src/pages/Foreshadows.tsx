...
const getStatusConfig = (status?: string) => {
  return STATUS_CONFIG[status as ForeshadowStatus] || {
    label: status || '未知状态',
    color: 'default',
...
const getCategoryConfig = (category?: string) => {
  return category ? CATEGORY_CONFIG[category] || { label: category, color: 'default' } : null;
};
  ...
  const columns = [
    {
      ...
      key: 'status',
      width: 100,
      sorter: (a: Foreshadow, b: Foreshadow) => (statusOrder[a.status] ?? 999) - (statusOrder[b.status] ?? 999),
          ...
          <div>
            <Row gutter={[16, 16]}>
              <Col span={24}>
                ...
                <Space>
                  <Tag color={getStatusConfig(currentForeshadow.status).color}>
                    {getStatusConfig(currentForeshadow.status).label}
                  </Tag>
                  {currentForeshadow.is_long_term && <Tag color="purple">长线伏笔</Tag>}
                  {getCategoryConfig(currentForeshadow.category) && (
                    <Tag color={getCategoryConfig(currentForeshadow.category)?.color}>
                      {getCategoryConfig(currentForeshadow.category)?.label}
                    </Tag>
                  )}
                </Space>
              </Col>
