...
export default function ProjectList() {
  ...
  const handleEnterProject = async (project: Project) => {
    const hasImportedContent = (project.current_words || 0) > 0 || project.status !== 'planning';
    if (project.wizard_status === 'incomplete' && !hasImportedContent) {
  ...
  const handleGenerateCover = async (project: Project, overwrite: boolean = true) => {
    try {
      ...
      await projectApi.generateCover(project.id, 'manual', overwrite);
  ...
  const handleEditCover = async (project: Project, prompt: string) => {
    try {
      message.loading({ content: `正在修改《${project.title}》封面...`, key: `cover-${project.id}` });
      await projectApi.editCover(project.id, prompt);
      message.success({ content: `《${project.title}》封面改图成功`, key: `cover-${project.id}` });
      await refreshProjects();
    } catch (error) {
      console.error('封面改图失败:', error);
      message.error({ content: `《${project.title}》封面改图失败`, key: `cover-${project.id}` });
    }
  };
  ...
  return (
    <div style={{
      ...
      background: `
        linear-gradient(${alphaColor(paper, 0.72)}, ${alphaColor(paper, 0.72)}),
        url(${zhuqueAssetUrls.paperTexture}),
        linear-gradient(180deg, ${alphaColor(paper, 0.82)} 0%, ${token.colorBgLayout} 42%)
      `,
              ...
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, overflow: 'hidden' }}>
                  <div style={{
                    width: 30,
                    height: 30,
                    background: `linear-gradient(135deg, ${alphaColor(token.colorWhite, 0.24)} 0%, ${alphaColor(zhuqueColors.gold, 0.34)} 100%)`,
                    borderRadius: 8,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
      ...
      <div style={{
        background: `linear-gradient(135deg, ${teal} 0%, #3D716D 72%, ${alphaColor(cinnabar, 0.72)} 100%)`,
          ...
          title={
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{
                ...
                <span style={{ fontWeight: 600, fontSize: 16, fontFamily: token.fontFamily }}>{APP_NAME}</span>
            </div>
          }
          placement="left"
        ...
        <div
          ...
          style={{
            ...
            background: activeView === 'projects'
              ? `
                linear-gradient(${alphaColor(paper, 0.64)}, ${alphaColor(paper, 0.64)}),
                url(${zhuqueAssetUrls.paperTexture}),
                linear-gradient(${alphaColor(ink, 0.035)} 1px, transparent 1px),
                linear-gradient(90deg, ${alphaColor(ink, 0.03)} 1px, transparent 1px),
                linear-gradient(180deg, ${alphaColor(paper, 0.76)} 0%, ${token.colorBgLayout} 38%)
              `
              : token.colorBgLayout,
            backgroundSize: activeView === 'projects' ? 'auto, cover, 52px 52px, 52px 52px, auto' : undefined,
          }}
        >
          ...
          {activeView === 'projects' && (
            <BookshelfPage
              ...
              onOpenImportModal={() => changeView('book-import')}
