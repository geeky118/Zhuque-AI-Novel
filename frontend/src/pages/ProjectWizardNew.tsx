...
import { COMIC_STYLE_OPTIONS, DEFAULT_COMIC_STYLE } from '../constants/comicStyles';
...
export default function ProjectWizardNew() {
  ...
  const handleResumeGeneration = async (projectId: string) => {
    try {
      const response = await fetch(buildApiPath(`/projects/${projectId}`), {
        credentials: 'include'
      });
      if (!response.ok) {
      ...
      if (project.wizard_status !== 'incomplete' || hasImportedContent) {
        message.success('项目已有正文内容，正在进入项目...');
      ...
      const config: GenerationConfig = {
        ...
        comic_style: project.comic_style || DEFAULT_COMIC_STYLE,
        comic_style_prompt: project.comic_style_prompt || null,
      };
  ...
  const handleAutoGenerate = async (values: WizardBasicInfo) => {
    const config: GenerationConfig = {
      ...
      comic_style_prompt: values.comic_style_prompt || null,
  ...
  const renderForm = () => (
    <Card
      style={{
        ...
        boxShadow: `0 18px 42px ${alphaColor(zhuqueColors.ink, 0.1)}`,
      }}
    ...
    >
      ...
      <Form
        ...
        initialValues={{
          ...
          character_count: 5,
          target_words: 100000,
          comic_style: DEFAULT_COMIC_STYLE,
      ...
      >
        ...
        >
          <Select size="large" placeholder="选择统一漫画风格">
        ...
        <Form.Item
          noStyle
          shouldUpdate={(prev, next) => prev.comic_style !== next.comic_style}
        >
          {({ getFieldValue }) => {
            const option = COMIC_STYLE_OPTIONS.find(item => item.value === getFieldValue('comic_style'));
            return option ? (
              <div style={{
                marginTop: -12,
                marginBottom: 18,
        ...
        >
          <TextArea
            rows={3}
            placeholder="可选：补充固定的线条、上色、光影、镜头、人物比例等风格要求"
            showCount
            maxLength={500}
          />
        </Form.Item>
  ...
  return (
    <div style={{
      ...
      background: `
        ...
        linear-gradient(180deg, ${alphaColor(zhuqueColors.paper, 0.9)} 0%, ${token.colorBgBase} 54%)
      `,
      ...
      <div style={{
        ...
        top: 0,
        zIndex: 100,
        background: `linear-gradient(135deg, ${zhuqueColors.teal} 0%, ${zhuqueColors.tealLight} 68%, ${alphaColor(zhuqueColors.cinnabar, 0.76)} 100%)`,
        boxShadow: `0 8px 26px ${alphaColor(zhuqueColors.ink, 0.16)}`,
      }}>
        <div style={{
          maxWidth: 1200,
          margin: '0 auto',
