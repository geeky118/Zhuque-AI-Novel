...
import { zhuqueColors } from '../theme/zhuqueTokens';
...
const MAX_TXT_SIZE_BYTES = 50 * 1024 * 1024;
...
export default function BookImport() {
  ...
  const [extractMode, setExtractMode] = useState<BookImportExtractMode>('full');
  ...
  useEffect(() => {
    ...
    if (cache) {
      ...
      if (cacheAgeMs > 6 * 60 * 60 * 1000) {
        ...
        setImportedProjectId(cache.importedProjectId ?? null);
        setExtractMode(cache.extractMode ?? 'full');
  ...
  useEffect(() => {
    ...
    // 导入完成后保留最小恢复信息，避免后续回到页面时丢失已落库的 project_id
    if (isApplyComplete) {
      saveBookImportCache({
        taskId: null,
        taskStatus: null,
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      try {
        const status = await bookImportApi.getTaskStatus(taskId, { silent: true });
  ...
  const startTask = async () => {
    ...
    try {
      ...
      message.error(detail ? `创建拆书任务失败：${detail}` : '创建拆书任务失败');
    } finally {
        ...
        payload,
        {
          ...
          onResult: (result) => {
            ...
            setTimeout(() => {
              setFailedSteps(prev => {
                if (prev.length === 0) {
                  message.success('导入已完成，章节分析和项目补全已在后台执行');
                  setTimeout(() => {
        ...
        stepsToRetry,
        {
          ...
          onResult: (result) => {
            if (result.still_failed && result.still_failed.length > 0) {
              ...
              clearBookImportCache();
              const projectId = result.project_id || importedProjectId;
              if (projectId) {
                setTimeout(() => {
  ...
  }, [taskId, failedSteps, navigate, importedProjectId]);

  const skipFailedSteps = useCallback(() => {
    setFailedSteps([]);
    clearBookImportCache();
    const projectId = importedProjectId;
    if (projectId) {
      message.info('已跳过失败步骤，正在跳转到项目...');
      navigate(`/project/${projectId}/chapters`);
    }
  }, [navigate, importedProjectId]);

  const resumePostImport = useCallback(async () => {
    const projectId = importedProjectId;
    if (!projectId) {
      message.warning('没有可继续的项目ID');
      return;
    }

    try {
      await bookImportApi.resumeFollowup(projectId);
      message.success('已重新启动后台补全任务');
    } catch (error) {
      console.error('重新启动后台补全失败:', error);
      message.error('重新启动后台补全失败');
    }
  }, [importedProjectId]);
  ...
  const analysisDossier = preview?.analysis_dossier;
  ...
  return (
    <div
      style={{
        ...
        overflow: 'auto',
        background: `
          linear-gradient(${alphaColor(zhuqueColors.ink, 0.032)} 1px, transparent 1px),
          linear-gradient(90deg, ${alphaColor(zhuqueColors.ink, 0.028)} 1px, transparent 1px),
          linear-gradient(180deg, ${alphaColor(zhuqueColors.paper, 0.95)} 0%, ${token.colorBgLayout} 58%)
        `,
    ...
    >
      <div style={{ maxWidth: 1400, margin: '0 auto', width: '100%' }}>
        <Card
          variant="borderless"
          style={{
            background: `linear-gradient(135deg, ${zhuqueColors.teal} 0%, ${zhuqueColors.tealLight} 68%, ${alphaColor(zhuqueColors.cinnabar, 0.78)} 100%)`,
            borderRadius: isMobile ? 14 : 16,
            boxShadow: `0 18px 42px ${alphaColor(zhuqueColors.ink, 0.12)}`,
            marginBottom: isMobile ? 14 : 16,
            border: `1px solid ${alphaColor(zhuqueColors.cinnabarDeep, 0.12)}`,
            position: 'relative',
            overflow: 'hidden',
          }}
        >
          <Row align="middle" justify="space-between" gutter={[16, 16]} style={{ position: 'relative', zIndex: 1 }}>
            <Col xs={24} sm={12}>
              <Space direction="vertical" size={4}>
                <Title level={isMobile ? 3 : 2} style={{ margin: 0, color: token.colorWhite, letterSpacing: 0 }}>
            ...
            <Col xs={24} sm={12}>
              ...
              >
                <Tag
                  style={{
                    marginInlineEnd: 0,
                    background: alphaColor(zhuqueColors.paperSoft, 0.92),
                    border: `1px solid ${alphaColor(zhuqueColors.gold, 0.42)}`,
                    color: zhuqueColors.cinnabarDeep,
                    fontWeight: 600,
                    borderRadius: 8,
                    paddingInline: 10,
                  }}
          ...
          <Card
            ...
            style={{
              ...
              boxShadow: `0 12px 28px ${alphaColor(zhuqueColors.ink, 0.08)}`,
            }}
      ...
      <Card title="上传 TXT 并开始解析" style={{ marginBottom: 16 }}>
        <Space direction="vertical" style={{ width: '100%' }} size={16}>
          <Dragger
            ...
            beforeUpload={(f) => {
              if (!f.name.toLowerCase().endsWith('.txt')) {
                message.warning('仅支持上传 TXT 文件');
                return Upload.LIST_IGNORE;
              }
              if (f.size > MAX_TXT_SIZE_BYTES) {
                message.warning('TXT 文件不能超过 50MB');
                return Upload.LIST_IGNORE;
              }
              clearBookImportCache();
              setTaskId(null);
          ...
          >
            ...
            <p className="ant-upload-hint">支持其他平台导出的 .txt，建议不超过 50MB</p>
          </Dragger>
          ...
          <Alert
            ...
            description={
              <div style={{ lineHeight: 1.8 }}>
                ...
                <div>4. 章节之间保留空行即可，系统会自动清理常见平台导出噪音。</div>
          ...
          {!preview ? (
            ...
            <div style={{ maxHeight: '60vh', overflowY: 'auto', paddingRight: 8 }}>
              ...
              {analysisDossier && (
                <Card
                  size="small"
                  title={`故事档案（人物 ${analysisDossier.characters.length} · 组织 ${analysisDossier.organizations.length} · 关系 ${analysisDossier.relationships.length} · 记忆 ${analysisDossier.memories.length}）`}
                >
                  <Space direction="vertical" style={{ width: '100%' }} size={12}>
                    {(analysisDossier.source_platform_notes.length > 0 || analysisDossier.world_notes.length > 0 || analysisDossier.characters.length > 0 || analysisDossier.relationships.length > 0 || analysisDossier.organizations.length > 0 || analysisDossier.memories.length > 0 || analysisDossier.foreshadows.length > 0) ? (
                      <>
                        {analysisDossier.source_platform_notes.length > 0 && (
                        <Alert
                          type="info"
                          showIcon
                          message="源文本识别"
                          description={analysisDossier.source_platform_notes.join('；')}
                        />
                        )}
                        {analysisDossier.world_notes.length > 0 && (
                        <Alert
                          type="success"
                          showIcon
                          message="世界与设定要点"
                          description={analysisDossier.world_notes.slice(0, 6).join('；')}
                        />
                        )}
                        <Collapse
                          items={[
                        {
                          key: 'characters',
                          label: `人物 ${analysisDossier.characters.length}`,
                          children: (
                            <List
                              size="small"
                              dataSource={analysisDossier.characters}
                              renderItem={(item) => (
                                <List.Item>
                                  <List.Item.Meta
                                    title={
                                      <Space wrap>
                                        ...
                                        <Tag>{item.role_type}</Tag>
                                        {item.first_seen_chapter ? <Tag color="blue">第 {item.first_seen_chapter} 章</Tag> : null}
                                      </Space>
                                    }
                                    description={
                                      <div style={{ whiteSpace: 'pre-wrap' }}>
                                        {item.personality || item.background || item.current_state || '已抽取到角色信息'}
                                      </div>
                                    }
                                  />
                                </List.Item>
                        ...
                        {
                          key: 'relationships',
                          label: `关系 ${analysisDossier.relationships.length}`,
                          children: (
                            <List
                              size="small"
                              dataSource={analysisDossier.relationships}
                              renderItem={(item) => (
                                <List.Item>
                                  <List.Item.Meta
                                    title={
                                      <Space wrap>
                                        ...
                                        <Tag color={item.intimacy_level < 0 ? 'red' : 'green'}>{item.relationship_type}</Tag>
                                      </Space>
                                    }
                        ...
                        {
                          key: 'organizations',
                          label: `组织 ${analysisDossier.organizations.length}`,
                          children: (
                            <List
                              size="small"
                              dataSource={analysisDossier.organizations}
                              renderItem={(item) => (
                                <List.Item>
                                  <List.Item.Meta
                                    title={
                                      <Space wrap>
                                        <Text strong>{item.name}</Text>
                                        {item.organization_type ? <Tag>{item.organization_type}</Tag> : null}
                                        <Tag color="blue">势力 {item.power_level}</Tag>
                                      </Space>
                                    }
                                    description={item.purpose || item.background || item.location || '已抽取到组织信息'}
                                  />
                                </List.Item>
                              )}
                            />
                        ...
                        {
                          key: 'memories',
                          label: `记忆 ${analysisDossier.memories.length}`,
                          children: (
                            <List
                              size="small"
                              dataSource={analysisDossier.memories}
                              renderItem={(item) => (
                                <List.Item>
                                  <List.Item.Meta
                                    title={
                        ...
                        {
                          key: 'foreshadows',
                          label: `伏笔 ${analysisDossier.foreshadows.length}`,
                          children: (
                            <List
                              size="small"
                              dataSource={analysisDossier.foreshadows}
                              renderItem={(item) => (
                                <List.Item>
                                  <List.Item.Meta
                                    title={
          ...
          {!failedSteps.length && !retrying && (
            <div style={{
              ...
              <Typography.Text type="secondary" style={{ fontSize: 13 }}>
                ...
                • 核心角色与相关组织<br />
                • 章节分析与记忆伏笔整理<br />
                {isApplyComplete ? '基础导入已完成，后台补全任务会继续执行。' : '请耐心等待，完成后将自动跳转。'}
              </Typography.Text>
              {isApplyComplete && importedProjectId && (
                <Space style={{ marginTop: 16 }}>
                  <Button onClick={resumePostImport}>
                    继续后台补全
                  </Button>
                  <Button type="primary" onClick={() => navigate(`/project/${importedProjectId}/chapters`)}>
                    进入项目
                  </Button>
                </Space>
              )}
