...
export default function SettingsPage() {
  ...
  `;
  const headerBackground = `linear-gradient(135deg, ${zhuqueColors.teal} 0%, ${zhuqueColors.tealLight} 68%, ${alphaColor(zhuqueColors.cinnabar, 0.78)} 100%)`;
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      form.setFieldsValue({
        ...
        image_text_language: settings.image_text_language || defaultCoverSettings.image_text_language,
      });
  ...
  const mumuTextDefaultUrl = '';
  const mumuCoverBaseUrlOptions = [
    { value: '', label: '自定义图片 API 地址', defaultModel: '' },
  ];
  ...
  const getCoverBaseUrlPlaceholder = () => {
    if (selectedCoverProvider === 'openai') {
      return 'https://api.openai.com/v1';
    }
    if (selectedCoverProvider === 'grok') {
      return 'https://api.x.ai/v1';
    }
    return 'https://generativelanguage.googleapis.com/v1beta';
  };

  const getCoverModelPlaceholder = () => {
    if (selectedCoverProvider === 'mumu') {
  ...
  const handleCoverProviderChange = (value: string) => {
    ...
    if (provider.value === 'openai') {
      nextValues.cover_api_base_url = '';
      nextValues.cover_image_model = '';
    } else if (provider.defaultUrl) {
      nextValues.cover_api_base_url = provider.defaultUrl;
    }
    if (provider.value === 'mumu') {
      nextValues.cover_api_key = '';
    }
    if (provider.value !== 'openai' && provider.defaultModel) {
  ...
  const handleCoverTestConnection = async () => {
    ...
    try {
      ...
      if (result.success) {
        message.success('图片接口测试成功');
      } else {
        message.error(result.message || '图片接口测试失败');
      }
    } catch (error) {
        ...
        <div style={{
          ...
          <Card
            ...
            style={{
              ...
              boxShadow: `0 18px 42px ${alphaColor(zhuqueColors.ink, 0.08)}`,
              flex: 1,
            }}
            styles={{
              body: {
                            ...
                            <Alert
                              ...
                              description={
                                <Space direction="vertical" size={8} style={{ width: '100%' }}>
                                  <Text>
                                    已自动填入专属地址，API Key 需由对应服务方提供后再填写。
                                  </Text>
                                </Space>
                              }
                              style={{ marginBottom: 16 }}
                  ...
                  children: (
                    <Spin spinning={initialLoading}>
                      <Form form={form} layout="vertical" onFinish={handleSave} autoComplete="off">
                        ...
                        <Form.Item label="图片生成功能" name="cover_enabled" style={{ marginBottom: 16 }}>
                          <Select
                            size={isMobile ? 'middle' : 'large'}
                            onChange={() => setCoverTestResult(null)}
                            options={[
                              { value: true, label: '启用图片生成' },
                              { value: false, label: '停用图片生成' },
                            ]}
                          />
                        </Form.Item>

                        <Form.Item label="图片 Provider" name="cover_api_provider" rules={[{ required: true, message: '请选择图片 Provider' }]}>
                        ...
                        {selectedCoverProvider === 'mumu' && (
                          <Alert
                            ...
                            description={
                              <Space direction="vertical" size={8} style={{ width: '100%' }}>
                                <Text>
                                  已固定提供 朱雀API 图片接口地址选项，切换地址时会自动带出推荐模型。API Key 需由对应服务方提供后填写。
                                </Text>
                              </Space>
                            }
                            style={{ marginBottom: 16 }}
                        ...
                        <Form.Item label="图片 API Key" name="cover_api_key" rules={[{ required: true, message: '请输入图片 API Key' }]}>
                          <Input.Password size={isMobile ? 'middle' : 'large'} placeholder={selectedCoverProvider === 'mumu' ? '请输入 朱雀API Key' : '输入图片 API Key'} autoComplete="new-password" />
                        </Form.Item>

                        <Form.Item label="图片 API 地址" name="cover_api_base_url" rules={[{ required: true, message: '请输入图片 API 地址' }, { type: 'url', message: '请输入有效的URL' }]}>
                          {selectedCoverProvider === 'mumu' ? (
                            <Select
                              size={isMobile ? 'middle' : 'large'}
                            ...
                            <Input size={isMobile ? 'middle' : 'large'} placeholder={getCoverBaseUrlPlaceholder()} />
                          )}
                        </Form.Item>

                        <Form.Item label="图片模型" name="cover_image_model" rules={[{ required: true, message: '请输入图片模型名称' }]}>
                          <Input
                            size={isMobile ? 'middle' : 'large'}
                            placeholder={getCoverModelPlaceholder()}
                          />
                        </Form.Item>

                        <Form.Item label="图片文字" name="image_text_language" rules={[{ required: true, message: '请选择图片文字语言' }]}>
                          <Select
                            size={isMobile ? 'middle' : 'large'}
                            onChange={() => setCoverTestResult(null)}
                            options={[
                        ...
                        <Form.Item style={{ marginBottom: 0, marginTop: 24 }}>
                          <Space wrap style={{ width: '100%', justifyContent: 'space-between' }}>
                            <Space wrap>
                              ...
                              >
                                {testingCoverApi ? '测试中...' : '测试图片接口'}
                              </Button>
                              <Button icon={<ReloadOutlined />} onClick={handleReset}>重置</Button>
                            </Space>
                            <Button type="primary" icon={<SaveOutlined />} htmlType="submit" loading={loading}>保存图片配置</Button>
                          </Space>
                        </Form.Item>
                      </Form>
                    </Spin>
              ...
              <Col xs={24} sm={8}>
                ...
                >
                  <Select placeholder="选择提供商" onChange={handlePresetProviderChange}>
                    <Select.Option value="mumu">朱雀API</Select.Option>
