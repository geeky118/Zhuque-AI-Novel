...
const getNodeSize = (node: Node) => {
  ...
  const width =
    typeof style?.width === 'number'
      ? style.width
      : Number(style?.width ?? 140) || 140;
  const height =
    typeof style?.height === 'number'
      ? style.height
      : Number(style?.height ?? 60) || 60;
...
export default function RelationshipGraph() {
  ...
  const loadRelationshipTypes = async () => {
    try {
      const relationshipTypesData = await api.get<unknown, RelationshipType[]>('/relationships/types');
  ...
  const loadGraphData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [graphData, characters, careersData] = await Promise.all([
        api.get<unknown, GraphData>(`/relationships/graph/${projectId}`),
        characterApi.getCharacters(projectId),
        api.get<unknown, CareerListResponse>('/careers', { params: { project_id: projectId } }),
      ]);
      ...
      characters.forEach((item) => {
        detailMap[item.id] = item as unknown as CharacterDetail;
      });
  ...
  const loadNodeDetail = async (nodeId: string) => {
    ...
    try {
      const characterDetail = await api.get<unknown, CharacterDetail>(`/characters/${nodeId}`);
      setNodeDetail(characterDetail);
    } catch (error) {
