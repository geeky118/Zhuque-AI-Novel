...
import api, { characterApi } from '../services/api';
...
export default function Organizations() {
  ...
  const loadOrganizations = useCallback(async () => {
    setLoading(true);
    try {
      const organizationsData = await api.get<unknown, Organization[]>(`/organizations/project/${projectId}`);
      setOrganizations(organizationsData);
      if (organizationsData.length > 0 && !selectedOrg) {
        setSelectedOrg(organizationsData[0]);
        loadMembers(organizationsData[0].id);
      }
    } catch (error) {
  ...
  const loadCharacters = useCallback(async () => {
    try {
      const characterItems = await characterApi.getCharacters(projectId || '');
  ...
  const loadMembers = async (orgId: string) => {
    try {
      const membersData = await api.get<unknown, OrganizationMember[]>(`/organizations/${orgId}/members`);
  ...
  const handleAddMember = async (values: Record<string, unknown>) => {
    ...
    try {
      await api.post(`/organizations/${selectedOrg.id}/members`, values);
  ...
  const handleUpdateMember = async (values: Record<string, unknown>) => {
    ...
    try {
      await api.put(`/organizations/members/${editingMember.id}`, values);
      ...
      >
        <Form
          ...
          onFinish={async (values) => {
            if (!selectedOrg) return;
            try {
              await api.put(`/organizations/${selectedOrg.id}`, values);
              ...
              // 更新当前选中的组织详情
              const updatedOrg = organizationsData.find((org: Organization) => org.id === selectedOrg.id);
              if (updatedOrg) {
