...
import type {
  ...
  CharacterImageInitializeResponse,
  CharacterImageState,
  CharacterImageVariantCreateRequest,
  CharacterImageVariantListResponse,
  CharacterImageVariantUpdateRequest,
  BibleImageItem,
  ...
  ComicBatchGenerateResponse,
  ComicBatchGenerateStatusResponse,
  ComicFullPipelineBatchGenerateRequest,
  ComicFullPipelineBatchGenerateResponse,
...
const COVER_IMAGE_REQUEST_TIMEOUT = 10 * 60 * 1000;

const withSilentRequest = (options?: RequestToastOptions): SilentRequestConfig | undefined => {
  if (!options?.silent) {
    return undefined;
  }
  return { suppressErrorToast: true };
};
...
const extractApiErrorMessage = (data: unknown): string | null => {
  ...
  if (typeof data === 'string') {
    ...
    const isHtml = /^<!doctype html/i.test(trimmed) || /^<html/i.test(trimmed) || trimmed.startsWith('<');
    if (isHtml) {
      return null;
    }
    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case 401: {
          ...
          const isUnauthenticated = backendDetail ? unauthenticatedDetails.includes(backendDetail) : false;

          errorMessage = backendDetail || '登录状态已失效，请重新登录';

          const loginPath = buildAppPath('/login');
          if (isUnauthenticated && window.location.pathname !== loginPath) {
            window.location.href = loginPath;
          }
          break;
        }
        case 403:
          errorMessage = backendMessage || '没有权限访问';
          break;
        case 404:
          errorMessage = backendMessage || '请求的资源不存在';
          break;
        case 422:
          errorMessage = backendMessage || '请求参数验证失败';
          if (typeof data === 'object' && data && 'errors' in data) {
            console.error('验证错误详情:', (data as { errors?: unknown }).errors);
          }
          break;
        case 500:
          ...
          break;
        case 503:
          errorMessage = backendMessage || '服务暂时不可用，请稍后重试';
          break;
        default:
...
export const promptTemplateApi = {
  ...
  createOrUpdate: (data: {
    ...
    parameters?: string;
    is_active: boolean;
  }) => api.post('/prompt-templates', data),

  reset: (templateKey: string) => api.post(`/prompt-templates/${templateKey}/reset`),

  update: (templateKey: string, data: {
    template_name?: string;
    template_content?: string;
    description?: string;
    category?: string;
    parameters?: string;
    is_active?: boolean;
  }) => api.put(`/prompt-templates/${templateKey}`, data),

  exportAll: () => api.post('/prompt-templates/export'),

  importAll: (data: unknown) => api.post('/prompt-templates/import', data),
};

export const projectApi = {
  getProjects: () => api.get<unknown, Project[]>('/projects'),

  getProject: (id: string, options?: RequestToastOptions) =>
    api.get<unknown, Project>(`/projects/${id}`, withSilentRequest(options)),
  ...
  getComicStyleOptions: () =>
    api.get<unknown, {
      default: string;
      items: Array<{ value: string; label: string; prompt: string }>;
    }>('/projects/comic-style/options'),

  deleteProject: (id: string) => api.delete(`/projects/${id}`),

  generateCover: (id: string, trigger: 'manual', overwrite: boolean = true) => {
    if (trigger !== 'manual') {
      throw new Error('Cover generation must be triggered manually');
    }
    return api.post<unknown, {
      ...
      model?: string;
      message: string;
    }>(`/projects/${id}/cover/generate`, { overwrite }, { timeout: COVER_IMAGE_REQUEST_TIMEOUT });
  },

  editCover: (id: string, prompt: string) =>
    api.post<unknown, {
      project_id: string;
      cover_status: string;
      cover_image_url?: string;
      ...
      model?: string;
      message: string;
    }>(`/projects/${id}/cover/edit`, { prompt }, { timeout: COVER_IMAGE_REQUEST_TIMEOUT }),

  downloadCover: async (id: string, filename?: string) => {
    const response = await axios.get(buildApiPath(`/projects/${id}/cover/download`), {
      responseType: 'blob',
      withCredentials: true,
    });
  ...
  exportProject: (id: string) => {
    window.open(buildApiPath(`/projects/${id}/export`), '_blank');
  },
  ...
  exportProjectData: async (id: string, options: {
    ...
    const response = await axios.post(
      buildApiPath(`/projects/${id}/export-data`),
      options,
      {
        responseType: 'blob',
        headers: {
...
export const bookImportApi = {
  ...
  getTaskStatus: (taskId: string, options?: RequestToastOptions) =>
    api.get<unknown, BookImportTask>(`/book-import/tasks/${taskId}`, withSilentRequest(options)),
  ...
  applyImportStream: (
    ...
    buildApiPath(`/book-import/tasks/${taskId}/apply-stream`),
    payload,
    options,
  ),
  ...
  retryFailedStepsStream: (
    ...
    buildApiPath(`/book-import/tasks/${taskId}/retry-stream`),
    { steps },
    options,
  ),

  resumePostImport: (projectId: string) =>
    api.post<unknown, {
      success: boolean;
      project_id: string;
      ...
      message: string;
    }>(`/book-import/projects/${projectId}/resume`),

  getFollowupStatus: (projectId: string, options?: RequestToastOptions) =>
    api.get<unknown, BookImportFollowupStatus>(
      `/book-import/projects/${projectId}/followup-status`,
      withSilentRequest(options),
    ),

  resumeFollowup: (projectId: string) =>
    api.post<unknown, BookImportFollowupStatus>(`/book-import/projects/${projectId}/followup/resume`),
...
export const outlineApi = {
  ...
  createBatchExpandTask: (data: BatchOutlineExpansionRequest) =>
    api.post<unknown, import('../types').BatchOutlineExpansionTaskCreateResponse>('/outlines/batch-expand-tasks', data),

  getBatchExpandTaskStatus: (taskId: string) =>
    api.get<unknown, import('../types').BatchOutlineExpansionTaskStatus>(`/outlines/batch-expand-tasks/${taskId}`),
};

export const characterApi = {
  getCharacters: (projectId: string, options?: RequestToastOptions) =>
    api.get<unknown, { total: number; items: Character[] }>(
  ...
  exportCharacters: async (characterIds: string[]) => {
    const response = await axios.post(
      buildApiPath('/characters/export'),
  ...
  batchUpdateVisualBible: (projectId: string, overwrite: boolean = false) =>
    api.post<unknown, { status: string; total: number; message?: string }>(
      `/characters/projects/${projectId}/visual-bible/batch-update?overwrite=${overwrite}`
    ),

  getBatchVisualBibleStatus: (projectId: string, options?: RequestToastOptions) =>
    api.get<unknown, { status: string; total?: number; completed?: number; failed?: number }>(
      `/characters/projects/${projectId}/visual-bible/batch-status`,
      withSilentRequest(options),
    ),

  regenerateVisualBible: (characterId: string) =>
    api.post<unknown, { status: string; visual_bible: Record<string, unknown> }>(
      `/characters/${characterId}/visual-bible/regenerate`
    ),
};

export const characterImageApi = {
  getProjectStates: (projectId: string, options?: RequestToastOptions) =>
    api.get<unknown, { project_id: string; total: number; items: CharacterImageState[] }>(
      `/character-images/projects/${projectId}`,
      withSilentRequest(options),
    ).then(res => res.items),

  getCharacterState: (characterId: string, options?: RequestToastOptions) =>
    api.get<unknown, CharacterImageState>(
      `/character-images/characters/${characterId}`,
      withSilentRequest(options),
    ),

  getCharacterVariants: (characterId: string, options?: RequestToastOptions) =>
    api.get<unknown, CharacterImageVariantListResponse>(
      `/character-images/characters/${characterId}/variants`,
      withSilentRequest(options),
    ),

  updatePrompt: (characterId: string, prompt: string) =>
    api.put<unknown, CharacterImageActionResponse>(
      `/character-images/characters/${characterId}/prompt`,
      { prompt }
    ),

  createVariant: (characterId: string, data: CharacterImageVariantCreateRequest) =>
    api.post<unknown, CharacterImageActionResponse>(
      `/character-images/characters/${characterId}/variants`,
      data
    ),

  updateVariant: (characterId: string, variantKey: string, data: CharacterImageVariantUpdateRequest) =>
    api.put<unknown, CharacterImageActionResponse>(
      `/character-images/characters/${characterId}/variants/${variantKey}`,
      data
    ),

  generateImage: (characterId: string, overwrite: boolean = true) =>
    api.post<unknown, CharacterImageActionResponse>(
      `/character-images/characters/${characterId}/generate`,
      { overwrite }
    ),

  generateVariantImage: (characterId: string, variantKey: string, overwrite: boolean = true) =>
    api.post<unknown, CharacterImageActionResponse>(
      `/character-images/characters/${characterId}/variants/${variantKey}/generate`,
      { overwrite }
    ),

  editImage: (characterId: string, prompt: string) =>
    api.post<unknown, CharacterImageActionResponse>(
      `/character-images/characters/${characterId}/edit`,
      { prompt }
    ),

  editVariantImage: (characterId: string, variantKey: string, prompt: string) =>
    api.post<unknown, CharacterImageActionResponse>(
      `/character-images/characters/${characterId}/variants/${variantKey}/edit`,
      { prompt }
    ),

  deleteVariant: (characterId: string, variantKey: string) =>
    api.delete<unknown, { message: string; variant_key: string }>(
      `/character-images/characters/${characterId}/variants/${variantKey}`
    ),

  initializeProject: (
    projectId: string,
    payload?: { overwrite?: boolean; limit?: number }
  ) => api.post<unknown, CharacterImageInitializeResponse>(
    `/character-images/projects/${projectId}/initialize`,
    payload ?? { overwrite: false }
  ),

  getBibleImages: (characterId: string, options?: RequestToastOptions) =>
    api.get<unknown, BibleImageListResponse>(
      `/character-images/characters/${characterId}/bible/images`,
      withSilentRequest(options),
    ),

  generateBibleImages: (characterId: string, params?: { angles?: string[]; max_concurrent?: number; overwrite?: boolean }) =>
    api.post<unknown, { status: string; total: number }>(
      `/character-images/characters/${characterId}/bible/generate`,
      params ?? {}
    ),

  deleteBibleImage: (characterId: string, fileName: string) =>
    api.delete<unknown, { status: string; file_name: string }>(
      `/character-images/characters/${characterId}/bible/images/${encodeURIComponent(fileName)}`
    ),

  regenerateBibleImage: (characterId: string, fileName: string, prompt: string) =>
    api.post<unknown, BibleImageItem>(
      `/character-images/characters/${characterId}/bible/images/${encodeURIComponent(fileName)}/regenerate`,
      { prompt }
    ),
};

export const chapterApi = {
  getChapters: (projectId: string, options?: RequestToastOptions) =>
    api.get<unknown, { total: number; items: Chapter[] }>(
  ...
  getAnalysisStatus: (chapterId: string, options?: RequestToastOptions) =>
    api.get<unknown, import('../types').AnalysisTask>(`/chapters/${chapterId}/analysis/status`, withSilentRequest(options)),

  getAnalysisResult: (chapterId: string) =>
    api.get<unknown, ChapterAnalysisResponse>(`/chapters/${chapterId}/analysis`),

  triggerAnalysis: (chapterId: string) =>
    api.post<unknown, TriggerAnalysisResponse>(`/chapters/${chapterId}/analyze`),
  ...
  getBatchAnalysisStatuses: (projectId: string, chapterIds?: string[], options?: RequestToastOptions) =>
    api.post<unknown, BatchAnalysisStatusResponse>(
      `/chapters/project/${projectId}/analysis/statuses`,
      {
  ...
  partialRegenerateStream: (
    ...
    buildApiPath(`/chapters/${chapterId}/partial-regenerate-stream`),
    data,
    options
  ),
...
export const comicApi = {
  getProjectChapters: (projectId: string, options?: RequestToastOptions) =>
    api.get<unknown, ComicProjectResponse>(`/comics/projects/${projectId}`, withSilentRequest(options)),

  getContinuousRead: (projectId: string, options?: RequestToastOptions) =>
    api.get<unknown, ComicContinuousReadResponse>(`/comics/projects/${projectId}/read`, withSilentRequest(options)),

  getChapterCombined: (projectId: string, chapterNumber: number, options?: RequestToastOptions) =>
    api.get<unknown, ComicChapterCombinedResponse>(
      `/comics/projects/${projectId}/chapters/${chapterNumber}/combined`,
      withSilentRequest(options),
    ),

  getChapterRegenerationStatus: (projectId: string, chapterNumber: number) =>
    api.get<unknown, ComicChapterRegenerationStatusResponse>(
      `/comics/projects/${projectId}/chapters/${chapterNumber}/regeneration-status`
    ),
  ...
  regeneratePage: (projectId: string, chapterNumber: number, pageNumber: number) =>
    api.post<unknown, {
      ...
      detail?: string;
    }>(`/comics/projects/${projectId}/chapters/${chapterNumber}/pages/${pageNumber}/regenerate`),

  editPage: (projectId: string, chapterNumber: number, pageNumber: number, prompt: string) =>
    api.post<unknown, {
      status: string;
      detail?: string;
    }>(`/comics/projects/${projectId}/chapters/${chapterNumber}/pages/${pageNumber}/edit`, { prompt }),

  batchGenerateComics: (projectId: string, startChapterNumber: number, count: number, comicPageConcurrency?: number) =>
    api.post<unknown, ComicBatchGenerateResponse>(`/comics/projects/${projectId}/batch-generate`, {
      start_chapter_number: startChapterNumber,
      count,
      comic_page_concurrency: comicPageConcurrency,
    }),

  getComicBatchGenerateStatus: (projectId: string, taskId: string, options?: RequestToastOptions) =>
    api.get<unknown, ComicBatchGenerateStatusResponse>(
      `/comics/projects/${projectId}/batch-generate/${taskId}/status`,
      withSilentRequest(options),
    ),

  batchGenerateFullPipeline: (
    projectId: string,
    payload: ComicFullPipelineBatchGenerateRequest,
  ) =>
    api.post<unknown, ComicFullPipelineBatchGenerateResponse>(`/comics/projects/${projectId}/full-batch-generate`, payload),

  getFullPipelineBatchGenerateStatus: (projectId: string, taskId: string, options?: RequestToastOptions) =>
    api.get<unknown, ComicFullPipelineBatchStatusResponse>(
      `/comics/projects/${projectId}/full-batch-generate/${taskId}/status`,
      withSilentRequest(options),
    ),

  getPageImageUrl: (projectId: string, chapterNumber: number, pageNumber: number) =>
    buildApiPath(`/comics/projects/${projectId}/chapters/${chapterNumber}/pages/${pageNumber}`),

  generateChapterStoryboard: (projectId: string, chapterNumber: number, targetPages?: number) =>
    api.post<unknown, { task_id: string; status: string; message: string }>(
      `/comics/projects/${projectId}/chapters/${chapterNumber}/storyboard/generate`,
      { target_pages: targetPages || 10 }
    ),

  batchGenerateStoryboard: (projectId: string, startChapterNumber: number, count: number, targetPages?: number) =>
    api.post<unknown, { task_id: string; status: string; total: number; chapter_numbers: number[]; message: string }>(
      `/comics/projects/${projectId}/storyboard/batch-generate`,
      { start_chapter_number: startChapterNumber, count, target_pages: targetPages || 10 }
    ),

  getStoryboardGenerateStatus: (projectId: string, taskId: string, options?: RequestToastOptions) =>
    api.get<unknown, {
      task_id: string;
      project_id: string;
      type: string;
...
export const wizardStreamApi = {
  generateWorldBuildingStream: (
    ...
    buildApiPath('/wizard-stream/world-building'),
    data,
    options
  ),
  ...
  generateCharactersStream: (
    ...
    buildApiPath('/wizard-stream/characters'),
    data,
    options
  ),
  ...
  generateCareerSystemStream: (
    ...
    buildApiPath('/wizard-stream/career-system'),
    data,
    options
  ),
  ...
  generateCompleteOutlineStream: (
    ...
    buildApiPath('/wizard-stream/outline'),
    data,
    options
  ),
  ...
  updateWorldBuildingStream: (
    ...
    buildApiPath(`/wizard-stream/world-building/${projectId}`),
    data,
    options
  ),
  ...
  regenerateWorldBuildingStream: (
    ...
    buildApiPath(`/wizard-stream/world-building/${projectId}/regenerate`),
    data || {},
    options
  ),
  ...
  cleanupWizardDataStream: (
    ...
    buildApiPath(`/wizard-stream/cleanup/${projectId}`),
    {},
    options
  ),
};
