...
export const AIProjectGenerator: React.FC<AIProjectGeneratorProps> = ({
  ...
  const handleResumeGenerate = async (data: GenerationConfig, projectIdParam: string) => {
    try {
      ...
      // 获取项目信息,判断当前完成到哪一步
      const response = await fetch(buildApiPath(`/projects/${projectIdParam}`), {
        credentials: 'include'
      });
      if (!response.ok) {
    ...
    const worldResult = await wizardStreamApi.generateWorldBuildingStream(
      {
        ...
        comic_style: data.comic_style,
        comic_style_prompt: data.comic_style_prompt,
    ...
    const worldResult = await wizardStreamApi.generateWorldBuildingStream(
      {
        ...
        comic_style: generationData.comic_style,
        comic_style_prompt: generationData.comic_style_prompt,
