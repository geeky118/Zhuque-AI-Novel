...
export default function ChapterAnalysis({ chapterId, visible, onClose }: ChapterAnalysisProps) {
  ...
  const fetchAnalysisStatus = async () => {
    try {
      ...
      const taskData: AnalysisTask = await chapterApi.getAnalysisStatus(chapterId);
  ...
  const fetchAnalysisResult = async () => {
    try {
      const data: ChapterAnalysisResponse = await chapterApi.getAnalysisResult(chapterId);
      setAnalysis(data);
    } catch (err) {
      setError((err as Error).message);
    }
  ...
  const startPolling = () => {
    const pollInterval = setInterval(async () => {
      try {
        const taskData: AnalysisTask = await chapterApi.getAnalysisStatus(chapterId, { silent: true });
        setTask(taskData);
  ...
  const triggerAnalysis = async () => {
    try {
      ...
      await chapterApi.triggerAnalysis(chapterId);
