...
export function useCharacterSync() {
  ...
  // 刷新角色列表
  const refreshCharacters = useCallback(async (projectId?: string, options?: { silent?: boolean }) => {
    const id = projectId || currentProject?.id;
    if (!id) return [];

    try {
      const data = await characterApi.getCharacters(id, { silent: options?.silent });
...
export function useChapterSync() {
  ...
  // 刷新章节列表
  const refreshChapters = useCallback(async (projectId?: string, options?: { silent?: boolean }) => {
    const id = projectId || currentProject?.id;
    if (!id) return [];

    try {
      const data = await chapterApi.getChapters(id, { silent: options?.silent });
  ...
  const generateChapterContentStream = useCallback(async (
    ...
    try {
      // 使用fetch处理流式响应
      const response = await fetch(buildApiPath(`/chapters/${chapterId}/generate-stream`), {
        method: 'POST',
        headers: {
