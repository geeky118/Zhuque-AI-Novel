...
import { zhuqueAssetUrls } from './zhuqueAssets';
...
export const ThemeProvider = ({ children }: PropsWithChildren) => {
  ...
  useLayoutEffect(() => {
    ...
    root.style.setProperty('--zq-paper-texture-url', `url("${zhuqueAssetUrls.paperTexture}")`);
      ...
      <ConfigProvider
        ...
        renderEmpty={() => (
          <div style={{ padding: '36px 16px', textAlign: 'center' }}>
            <img
              src={zhuqueAssetUrls.emptyState}
              alt=""
              style={{
                width: 180,
                maxWidth: '52%',
                opacity: resolvedMode === 'dark' ? 0.62 : 0.86,
                filter: resolvedMode === 'dark' ? 'saturate(0.92) brightness(0.86)' : 'none',
                marginBottom: 10,
              }}
            />
            <div style={{ color: 'var(--ant-color-text-tertiary)', fontSize: 13 }}>暂无数据</div>
          </div>
        )}
      >
        {children}
